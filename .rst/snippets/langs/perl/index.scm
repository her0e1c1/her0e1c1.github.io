...
;; # mapでなくforで記述perl -e 'for(<*.txt>){$a=$_; s/\.txt/\.csv/; rename $a, $_}'
...
;; perl -E '@a=(split " ", "a b c d e")[-1, 0]; say @a'  # eamysqldump -u USER DB TABLE -t --compact --skip-extended-insert |perl -plE 's/(?<=VALUES \()(\d+)/$1+10000/e'
...
;; perl -E 'for $i (qw/a b c/){for(qw/txt csv/){`touch $i.$_`}}'# // .. // operator
;; # 以下のコードと同じ
;; # my $process_this = 0;
;; # while (<$file>) {
...
;; # }
;; echo "a\nb\nSTART\nc\nEND\ne" |perl -nle 'print if /^START$/ .. /^END$/'perl -E 'say length shift' 1234567890perl -E '$_=shift; say if /(["''])abc\1/ ' '"abc"'  # "abc"
